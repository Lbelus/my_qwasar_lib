...
"""
"""
unoptimized need to use memoization

"""
def do_jump(stones, curr, last_jump):
    if curr == len(stones) - 1:
        return True
    for jump in [last_jump - 1, last_jump, last_jump + 1]:
        if jump > 0:
            next = stones[curr] + jump
            if next in stones:
                next_index = stones.index(next)
                if do_jump(stones, next_index, jump):
                    return True
    return False
...
def frog_jump(stones):
    ...
    curr = 1
    last_jump = 1
    return do_jump(stones, curr, last_jump)